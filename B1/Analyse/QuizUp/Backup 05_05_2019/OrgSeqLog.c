...
{
  return strcmp(load1->utilisateur.nom,load2->utilisateur.nom);
}
...
int searchFileSeqLog(char * fileName,char * value,long startpos,PayLoad * record)
{
  ...
  if (fichier!=NULL)
  {
    if(fread(&header,sizeof(Header),1,fichier)==1)
    {
      ...
      /*
      if(fseek(fichier,startpos,SEEK_SET)==0)
      {
        //Faire quoi?
      }
      */
        ...
        do
        {
          ...
          if(fseek(fichier,offset,SEEK_SET)==0)
          {
            if(fread(&temp,sizeof(EnrSeqLog),1,fichier)==1)
            {
              ...
              found=(strcmp(value,temp.payload.utilisateur.nom)==0); // Inserer la fonction de comparaison qui convient
              pos_cur=pos_next;
              pos_next=temp.offset;
              erreur=0;
            }
...
//I: Un pointeur sur le fichier a lire,la positionun pointeur sur l'enregistrement a lire et la position suivante
//P: Lis un enregistrement et la place dans le pointeur de payload
//O: 0 si tout est bon et -1 si il y a une erreur
int readFileSeqLog(char * fileName,long position,PayLoad * record,long *suivant,long *pPosition)
{
  FILE *fichier = NULL;
  Header header = {-2,-2};
  EnrSeqLog temp={{{0}},-1};
  ...
  if(fichier != NULL)
  {
    if(fread(&header,sizeof(Header),1,fichier)==1)
    {
      ...
      //Si le fichier n'est pas vide deplacer le curseur apres le header
      offset = sizeof(Header) + position*sizeof(EnrSeqLog);
      if(fseek(fichier,offset,SEEK_SET)==0)
      {
        if(pPosition != NULL)
        {
          *pPosition = ftell(fichier);
        }
        //Si on arrive a bien bouger le curseur
        if(fread(&temp,sizeof(EnrSeqLog),1,fichier)== 1)
        {
          //Si on a reussi a lire le fichier,recopier le payload dans payload*record
          memcpy(record,&(temp.payload),sizeof(PayLoad));
          *suivant = temp.offset;
          erreur = 0;
        }
