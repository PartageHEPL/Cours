...
//P:
//O:
int Encoder_Reponses(Globale *pVar_Globale)
{
	int i = 0,reponseDonnee = -1,temps,bonneReponse = -1,erreur = 0;
	int nbrThemes = 0,rConnecter = -1;
	Question Q1 = {0},Q2 = {0},Q3 = {0},Q4 = {0},Q5={0};
	int o1,o2,o3,o4;
	char theme[50] = {0};
	Reponse temp;
	Question *question;
	...
	system("cls");
	srand(time(NULL));
	rConnecter = Connecter(pVar_Globale);
	if(rConnecter != -1)
	{
		temp.idProfil = rConnecter;
		system("cls");
		printf("Voici la liste des themes:\n");
		printf("0. Tous les themes\n");
		nbrThemes = AfficherThemes(pVar_Globale->questions,pVar_Globale->nbrQuestions);
		if(nbrThemes == 0)
		{
			system("cls");
			printf("Il n'y a aucuns themes!\n");
		}
		else
		{	
			...
			if(strcmp(theme,"0") == 0)
			{
				//Faire 5 questions sur nimporte quel theme
				temp.idPartie = GetQuestionId(pVar_Globale->globale);

			}
			else
			{
				//Faire 5 question sur le theme choisi
				temp.idPartie = GetQuestionId(pVar_Globale->globale);
				//GetTheme(pVar_Globale->questions,choixTheme,&theme[0]);
				GetQuestions(pVar_Globale->questions,&theme[0],&Q1,&Q2,&Q3,&Q4,&Q5);
				for(i = 0; i < 5; i++)
				{
					if(i == 0)
					{
					...
					//Remplir la stuct reponse
					temp.idQuestion = question->idQuestion;
					printf("Question %d:\n",i);
					printf("%s\n\n",question->libelle);
					o1 = rand() % (4 - 1 + 1) + 1;
					do
					...
					//Afficher les proposition de reponses
					switch(o1)
					{
						case 1:
							...
							break;
						case 2:
							printf("1. %s\n",question->distracteur1);
							break;
						case 3:
							printf("1. %s\n",question->distracteur2);
							break;
						case 4:
							printf("1. %s\n",question->distracteur3);
							break;
					}
					switch(o2)
					{
					...
					//Demarer le timer
					clock_t start = clock();
					//Recpuerer la reponse
					printf("Quel est votre reponses? ");
					fflush(stdin);
					scanf("%d",&reponseDonnee);
					clock_t end = clock();
					temps = (int)(end - start) / CLOCKS_PER_SEC;
					temp.timestamp = end;

					if(reponseDonnee == -1)
					{
						//on dois faire un truc special
					}
					else
					{
						//Savoir si il a donner la bonne reponse
					...
					else if(temps >= 4 && temps < 5)
					{
						temp.points = 7;
					}
					else if(temps >= 5 && temps < 6)
					{
						temp.points = 6;
					}
					else if(temps >= 6 && temps < 7)
					{
						temp.points = 5;
					}
					else if(temps >= 7 && temps < 8)
					{
						temp.points = 4;
					}
					else if(temps >= 8 && temps < 9)
					{
						temp.points = 3;
					}
					else if(temps >= 9 && temps < 10)
					{
						temp.points = 2;
					}
					else if(temps >= 10 && temps < 11)
					{
						temp.points = 1;
					}
					else
					...
					temp.reponseDonnee = reponseDonnee;

					//Afficher la struct reponse
					//printf("idProfile: %ld\n",temp.idProfil);
					//printf("idQuestion: %ld\n",temp.idQuestion);
					//printf("idPartie: %ld\n",temp.idPartie);
					//printf("ReponseDonner: %d\n",temp.reponseDonnee);
					//printf("Points: %d\n",temp.points);
					//Enregistrer les reponses dans le fichier
					FILE *fichier = NULL;
					fichier = fopen(pVar_Globale->reponses,"ab+");
					if(fichier == NULL)
					{
						erreur = -1;
					}
...
int Encoder_Questions(Globale *pVar_Globale,int sauvegarder)
{
	...
	do
	{
		...
		fflush(stdin);
		gets(question.theme);
	}while(TesterTheme((char *)&(question.theme)) == -1 || Test_Imprimable((char *)&(question.theme)) == -1 || strlen(question.theme) == 0);	
	//Libelle
	do
	{
	...
	}while(Test_Imprimable((char *)(&question.libelle)) == -1 || strlen(question.libelle) == 0);
	//CorrectAnswer
	do
	{
	...
	}while(Test_Imprimable((char *)&(question.correctAnswer)) == -1 || strlen(question.correctAnswer) == 0);
	//Distracteur1
	do
	{
	...
	}while(Test_Imprimable((char *)&(question.distracteur1)) == -1 || strlen(question.distracteur1) == 0);
	//Distracteur2
	do
	{
	...
	}while(Test_Imprimable((char *)&(question.distracteur2)) == -1 || strlen(question.distracteur2) == 0);
	//Disctracteur3
	do
	{
	...
	}while(Test_Imprimable((char *)&(question.distracteur3)) == -1 || strlen(question.distracteur3) == 0);
...
//P:
//O:
void Afficher_Questions(Question *temp)
{
	printf("Question n%c%ld (ID) => %s\n",167,temp->idQuestion,temp->libelle);
	printf("Theme: %s\n",temp->theme);
	printf("CorrectAnswer : %s\n",temp->correctAnswer);
	printf("Distracteur1: %s\n",temp->distracteur1);
	printf("Distracteur2: %s\n",temp->distracteur2);
	printf("Distracteur3: %s\n",temp->distracteur3);
}
...
//P:
//O:
int AfficherThemes(char *filename,int nbrQuestions)
{
	int present,nbrTheme = 0;
...
int SupprimerQuestion(Globale *pVar_Globale,char *filename)
{
	int nbrQuestions;
	int erreur;
	int trouver = 0;
	...
	do
	{
		printf("\nQuel question voulez vous supprimer? ");
		fflush(stdin);
		scanf("%d",&ModifierID);
		printf("QuestionID : %d\n", pVar_Globale->questionID);
		printf("ModifierID : %d\n",ModifierID);
	}while(ModifierID > pVar_Globale->questionID);
	...
	{
		//Parcourir le fichier jusqu'a l'emlacement
		while(fread(&temp,sizeof(Question),1,fichier) == 1)
		{
			if(temp.idQuestion == ModifierID)
			{
				//On se trouve au bonne emplacement
				trouver = 1;
			}
		}
		if(trouver == 0)
		{
			printf("Impossible de trouver une question avec l'ID %d\n",ModifierID);
		}
		else
		{
			fseek(fichier,-(sizeof(Question)),SEEK_CUR);
			//Ecrire des 0 pour supprimer le profile
			if(fwrite(&vide,sizeof(Question),1,fichier) != 1)
			{
				erreur = -1;
			}
			fclose(fichier);
			nbrQuestions = InitialiserNbrQuestion(pVar_Globale->questions);
			nbrQuestions--;
			WriteNbrQuestions(pVar_Globale->questions,nbrQuestions);
		}
	}
	return erreur;	
}

int GetQuestionId(char *filename)
{
	int erreur = 0,ID = 0;
	int offset = sizeof(Header) + sizeof(EnrSeqLog) + sizeof(int) * 2 + sizeof(char) * 20 * 4 + sizeof(char) * 50 + sizeof(int) + sizeof(Question);
	FILE *fichier = NULL;
	fichier = fopen(filename,"rb+");
	if(fichier == NULL)
	{
	...
	{
		fseek(fichier,offset,SEEK_SET);
		if(fread(&ID,sizeof(int),1,fichier) != 1)
		{
			erreur = -1;
		}
		ID++;
			...
			if(header.PTO != -1)
			{
				...
				while(temp.offset != -1)
				{
					...
					if(strcmp(temp.payload.utilisateur.nom,pVar_Globale->nomConnexion)== 0 && strcmp(temp.payload.utilisateur.password,mdp)== 0)
					{
						...
						erreur = temp.payload.utilisateur.idProfil;
					}
				}
			}
		}
...
int GetTheme(char *filename,int idTheme,char *theme)
{
	int erreur = 0;
	Question temp;
	FILE *fichier = NULL;
	...
	else
	{
		while(fread(&temp,sizeof(Question),1,fichier) == 1)
		{
			if(temp.idQuestion == idTheme)
			{
				strcpy(theme,temp.theme);
			}
		}
...
int GetLibelle(char *filename,int id,char *libelle)
{
	int erreur = 0;
	Question temp;
	FILE *fichier = NULL;
	...
	else
	{
		while(fread(&temp,sizeof(Question),1,fichier) == 1)
		{
			if(temp.idQuestion == id)
			{
				strcpy(libelle,temp.libelle);
			}
		}
		fclose(fichier);
	}
...
int GetQuestions(char *filename,char *theme,Question *idQ1,Question *idQ2,Question *idQ3,Question *idQ4,Question *idQ5)
{
	srand(time(NULL));
	int erreur= 0;
	int i = 0,Q1,Q2,Q3,Q4,Q5;
	Question vec[100] = {0};
	int nbrQuestions = 0;
	Question temp;
	FILE *fichier = NULL;
	...
	else
	{
		...
		//Choisir pamis les questions de ce theme precis
		Q1 = (rand() % (nbrQuestions-1 - 0 + 1)) - 0;
		do
		{
			Q2 = (rand() % (nbrQuestions-1 - 0 + 1)) - 0;
		}while(Q2 == Q1);
		do
		{
			Q3 = (rand() % (nbrQuestions-1 - 0 + 1)) - 0;
		}while(Q3 == Q1 || Q3 == Q2);
		do
		{
			Q4 = (rand() % (nbrQuestions-1 - 0 + 1)) - 0;
		}while(Q4 == Q1 || Q4 == Q2 || Q4 == Q3);
		do
		{
			Q5 = (rand() % (nbrQuestions-1 - 0 + 1)) - 0;
...
int UpdateLastActivity(Globale *pVar_Globale,char *nom)
{
	int erreur = -1;
	long position;
	Header header = {0};
	...
	if(fichier != NULL)
	{
		...
		if(header.PTO != -1)
		{
			...
			while(temp.offset != -1)
			{
				...
				{
					temp.payload.utilisateur.lastActivity = time(NULL);
					FILE *fichier = NULL;
					fichier = fopen(pVar_Globale->profiles,"rb+");
					if(fichier != NULL)
					{
...
int AfficherPartie(Globale *pVar_Globale, int ID)
{
	int erreur = 0;
	/*
	Reponse temp;
	Reponse tRep[1000] = {0};
	Reponse *ptRep = &tRep[0];
	...
	FILE *fichier = NULL;
	fichier = fopen(pVar_Globale->reponses,"rb+");
	if(fichier == NULL)
	{
		erreur = -1;
	}
	else
	{
		printf("Test1\n");
		//Lire les reponses
		while(fread(&temp,sizeof(Reponse),1,fichier) == 1)
		{
			if(temp.idProfil == ID)
			{
				memcpy(ptRep,&temp,sizeof(Reponse));
				nbrReponses++;
				ptRep++;
			}
		}
...
void GetQuestion(Globale *pVar_Globale,int idQuestion,Question *qTemp)
 {
 	Question temp;
 	FILE *fichier = NULL;
 	fichier = fopen(pVar_Globale->questions,"rb+");
 	if(fichier != NULL)
 	{
 		while(fread(&temp,sizeof(Question),1,fichier) == 1)
 		{
 			if(idQuestion == temp.idQuestion)
 			{
 				memcpy(qTemp,&temp,sizeof(Question));
 				//qTemp = temp;
 			}
 		}
 		fclose(fichier);
...
//O: /
int Supprimer_Tous_Fichier(Globale *pVar_Globale)
{
	int erreur = 0;
	if(remove(pVar_Globale->profiles)== -1)
	{
		erreur = -1;
		printf("Erreur lors de la suppresion de %s!\n",pVar_Globale->profiles);
	}
	if(remove(pVar_Globale->questions)== -1)
	{
		erreur = -1;
		printf("Erreur lors de la suppresion de %s!\n",pVar_Globale->questions);
	}
	if(remove(pVar_Globale->reponses)== -1)
	{
		erreur = -1;
		printf("Erreur lors de la suppresion de %s!\n",pVar_Globale->reponses);
	}
	
	if(remove(pVar_Globale->globale) == -1)
	{
		erreur = -1;
		printf("Erreur lors de la suppresion de %s!\n",pVar_Globale->globale);
	}
	return erreur;
}
